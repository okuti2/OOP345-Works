...
int main(int argc, char** argv)
{
		...
		if (fs) {
			do
			{
				...
				if (fs)
				{
					...
					if (movieInfo[0] != '#')
					{
						movies[counter] = Movie(movieInfo);
						++counter;
					}
				}
			} while (fs);
	...
	catch(const std::exception &errMes){
		std::cerr << "** EXCEPTION: " << errMes.what() << std::endl;
	}
