...
namespace sdds
{
   ...
   TennisLog& TennisLog::operator=(const TennisLog& tennisLog) { // copy assignment operator
      if (this != &tennisLog) {
         delete[] m_tennisMatch;
         m_tennisMatch = new TennisMatch[tennisLog.m_numOfMatches + 1];
         if (tennisLog.m_numOfMatches != 0) {
            for (size_t i = 0; i < tennisLog.m_numOfMatches; i++) {
   ...
   TennisLog& TennisLog::operator=(TennisLog&& src) noexcept {
      if (this != &src) {
         delete[] m_tennisMatch;
         m_numOfMatches = src.m_numOfMatches;
